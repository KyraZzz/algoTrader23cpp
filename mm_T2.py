    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if self.active_orders < ACTIVE_ORDERS_LIMIT and other in self.top_bid_dic.keys() and other in self.top_ask_dic.keys():
                if self.pot_bid < POSITION_LIMIT and f_bid_p0 - (e_bid_p0 + TICK_SIZE_IN_CENTS) >= THRESHOLD * (e_bid_p0 + TICK_SIZE_IN_CENTS):
                ...
                if self.pot_ask > -POSITION_LIMIT and (e_ask_p0 - TICK_SIZE_IN_CENTS) - f_ask_p0 >= THRESHOLD * f_ask_p0:
